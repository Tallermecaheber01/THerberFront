...
export const sendVerificationCode2 = async (email) => {
  try {
    const response = await api.post('users/register/send-code', {
      correo: email,
    });
...
export const verifyCode2 = async (email, code) => {
  try {
    const response = await api.post('users/register/verify-code', {
      email,
      code,
    });
...
export const sendVerificationCode = async (email) => {
  try {
    const response = await api.post('users/recover-password/send-code', {
      correo: email,
    });
...
export const verifyCode = async (email, code) => {
  try {
    const response = await api.post('users/recover-password/verify-code', {
      email,
      code,
    });
...
export const resetPassword = async (email, newPassword) => {
  try {
    const response = await api.post('users/recover-password/reset', {
      email,
      newPassword,
    });
